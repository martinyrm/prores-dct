...
{
    int16_t prepost[128] = { 0 };
    for (int i = 0; i < 64; i++)
        block[i] *= qmat[i]; //each block and each qmat contains 64 signed int16s
	//multiply by qmat for dequantization
      
    for (int  i = 0; i < 64; i++)  //add dequantized block to prepost, which will later be written out to file
	prepost[i] = block[i];

    for (int i = 0; i < 8; i++)
        idctRowCondDC_extrashift_10(block + i*8, 2); //block[0,8,16,24,32,40,48,56,64]
    
    for (int i = 0; i < 8; i++) {
        block[i] += 8192;
        idctSparseCol_extrashift_10(block + i); //block[0,1,2,34,5,6,7,8]
    }
    
 
    for (int  i = 0; i < 64; i++) 
	prepost[i + 64] = block[i];  //add post-idct block to prepost

    FILE *fp2;	//write out all blocks to file
    fp2 = fopen("/Users/martinr/all_blocks_prepost.dat", "ab");
    fwrite(prepost, sizeof(int16_t), 128, fp2);          
    fflush(fp2);
    fsync(fileno(fp2));
    fclose(fp2);
}
...
static void prores_idct_put_10_c(uint16_t *out, ptrdiff_t linesize, int16_t *block, const int16_t *qmat)
{
    FILE *fp;
    uint16_t delim = 109;
     fp = fopen("/Users/martinr/allblocks_idct_preclip_preidct.dat", "ab");
        fwrite(block, sizeof(uint16_t), 64, fp);
        //fwrite(&delim, sizeof(uint16_t), 1 , fp);
        fclose(fp);
    prores_idct_10(block, qmat);
       
 
        //if (access("/Users/martinr/firstblock_idct_preclip.dat", F_OK) == 0) {
        //} else {
        fp = fopen("/Users/martinr/allblocks_idct_preclip.dat", "ab");
        fwrite(block, sizeof(uint16_t), 64, fp);
	//fwrite(&delim, sizeof(uint16_t), 1 , fp);
        fclose(fp);
    //}
